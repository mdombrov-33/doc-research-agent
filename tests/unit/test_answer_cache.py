...
def test_exact_hit_skips_the_embedding_call(client, embeddings):
    ...
    result = answer_cache.lookup("q", model="a/x", top_k=5)
...
def test_miss_returns_none(client, embeddings):
    assert answer_cache.lookup("q", model="a/x", top_k=5) is None


def test_lookup_filters_on_model_topk_and_current_version(client, embeddings):
    client.retrieve.return_value = [_point({"corpus_version": 7})]

    answer_cache.lookup("q", model="a/x", top_k=3)
...
def test_lookup_disabled_returns_none_without_touching_qdrant(client, embeddings, monkeypatch):
    monkeypatch.setattr(
        answer_cache, "get_settings", lambda: Settings(ANSWER_CACHE_ENABLED=False)
    )
...
def test_store_upserts_entry_with_current_version(client, embeddings):
    ...
    answer_cache.store(
        "q", answer="42", sources=[{"id": "S1"}], model="a/x", top_k=5
    )
